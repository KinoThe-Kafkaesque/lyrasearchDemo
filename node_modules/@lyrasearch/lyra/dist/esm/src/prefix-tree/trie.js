import { create as createNode, removeDocument, updateParent } from "./node";
...
export function insert(nodes, node, word, docId) {
...
export function contains(nodes, node, word) {
    const wordLength = word.length;
    for (let i = 0; i < wordLength; i++) {
        const char = word[i];
...
export function find(nodes, node, { term, exact, tolerance }) {
    const output = {};
...
export function removeDocumentByWord(nodes, node, word, docID, exact = true) {
    if (!word) {
        return false;
    }
...
export function removeWord(nodes, node, word) {
    if (!word) {
        return false;
    }
