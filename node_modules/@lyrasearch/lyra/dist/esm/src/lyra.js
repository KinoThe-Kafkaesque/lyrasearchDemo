...
import { create as createNode } from "./prefix-tree/node";
import { find as trieFind, insert as trieInsert, removeDocumentByWord } from "./prefix-tree/trie";
import { tokenize } from "./tokenizer";
import { SUPPORTED_LANGUAGES } from "./tokenizer/languages";
import { availableStopWords, stopWords } from "./tokenizer/stop-words";
import { getNanosecondsTime, includes, insertSortedValue, intersectTokenScores, sortTokenScorePredicate, uniqueId, } from "./utils";
...
export function create(properties) {
    const defaultLanguage = properties?.defaultLanguage?.toLowerCase() ?? "english";
 ...
 * });
 */
export function insert(lyra, doc, config) {
 ...
 */
export function remove(lyra, docID) {
    if (!lyra.tokenizer) {
 ...
 * });
 */
export function search(lyra, params, language) {
    if (!language) {
...
export function load(lyra, { index, docs, nodes, schema, frequencies, tokenOccurrencies }) {
    if (!lyra.edge) {
